...
namespace AIProducts.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("AIProducts") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "AIProducts", DisplayName = "AIProducts API", Resources = { "AIProducts" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["AIProducts_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["AIProducts_Web:RootUrl"]!.EnsureEndsWith('/');

            /* AIProducts_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["AIProducts_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["AIProducts_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["AIProducts_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // App Client
        var appClientId = configurationSection["AIProducts_App:ClientId"];
        if (!appClientId.IsNullOrWhiteSpace())
        {
                await CreateApplicationAsync(
                    ...
                    secret: configurationSection["AIProducts_App:ClientSecret"] ?? "1q2w3e*",
